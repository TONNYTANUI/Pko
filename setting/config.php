	...
	
{
	...
	public $password = "kipla321";
	public $dbname = "valleyview_database";
	public $connectdb;
	...
	public function t_check($t_username,$t_password)
	{
		$meadin_login_select = "SELECT * from teacher_info where t_username='$t_username' AND t_password='$t_password'";
		$meadmin_login_run = $this->connectdb->query($meadin_login_select);
		// var_dump($meadmin_login_run);
		if($meadmin_login_run){
		$meadmin_login_num = $meadmin_login_run->num_rows;
	...
	public function t_username($adminname)
	{
		$t_username_select = "SELECT * from teacher_info where t_username='$adminname'";
		$t_username_run = $this->connectdb->query($t_username_select);
		return $t_username_run;
	}
	public function t_updatepassword($password_update,$username)
	{
		$t_password_update = "UPDATE teacher_info set 	t_password='$password_update', t_username='$username'where t_id='$t_id'";
		$password_update_run = $this->connectdb->query($t_password_update);
		return $password_update_run;
	}
	////////////////////////End Teacher Info ------------//////////////////////
